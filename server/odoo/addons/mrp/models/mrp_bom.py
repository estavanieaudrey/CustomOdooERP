...
class MrpBom(models.Model):
    ...
    # gramasi_kertas_isi = fields.Float(string="Gramasi Kertas Isi")
    # harga_kertas_isi = fields.Float(string="Harga Kertas Isi per Kg")
    # jumlah_kertas_isi = fields.Float(string="Jumlah Kertas Isi (Rim)")
    # total_biaya_kertas_isi = fields.Float(string="Total Biaya Kertas Isi", compute='_compute_biaya_kertas_isi',
    #                                       store=True)
    # margin_profit = fields.Float(string="Margin Profit (%)", default=10)
    # harga_penawaran = fields.Float(string="Harga Penawaran", compute='_compute_harga_penawaran', store=True)
    #
    # @api.depends('gramasi_kertas_isi', 'harga_kertas_isi', 'jumlah_kertas_isi')
    # def _compute_biaya_kertas_isi(self):
    #     for record in self:
    #         # Rumus perhitungan Total Biaya Kertas
    #         record.total_biaya_kertas_isi = record.jumlah_kertas_isi * record.harga_kertas_isi
    #
    # @api.depends('total_biaya_kertas_isi', 'margin_profit')
    # def _compute_harga_penawaran(self):
    #     for record in self:
    #         # Rumus perhitungan Harga Penawaran
    #         record.harga_penawaran = record.total_biaya_kertas_isi * (1 + record.margin_profit / 100)
...
class MrpBomLine(models.Model):
    ...
    def _skip_bom_line(self, product, never_attribute_values=False):
        ...
        other_attribute_valid = product._match_all_variant_values(
            self.bom_product_template_attribute_value_ids.filtered(
        ...
        if not self.bom_product_template_attribute_value_ids.filtered(
                lambda a: a.attribute_id.create_variant == 'no_variant'):
